...
public class DragShotMover : MonoBehaviour
{
    ...
    /*
        Code found from: https://forum.unity3d.com/threads/drag-shot-mover-get-your-physics-objects-moving.86472/
    ...
    */
	...
	//things needed to check if the ball is moving
	...
	// lets other scripts see if the ball is moving
    ...
    void Update()
    {
        if (FreeLookCam.enablefreelook)
        {
            ...
            previousLocations[previousLocations.Length - 1] = ballTransform.position;

            // Check the distances between the points in your previous locations
            // if for a few updates there are no movement smaller than the threshold,
            // you can assume the ball isn't moving
            for (int i = 0; i < previousLocations.Length - 1; i++)
            {
                if (Vector3.Distance(previousLocations[i], previousLocations[i + 1]) >= noMovementThreshold)
                {
                    // The minimum has been detected between frames
                    isMoving = true;
                    break;
                }
                else
